...
public class CourseLearningService : ICourseLearningService
{
    ...
    //public async Task<ClassroomDto?> GetClassroomViewModelAsync(int studentId, int courseId, int? lessonId = null)
    //{
    ...
    //                //TotalDurationInMinutes = (int)Math.Round(c.Modules
    //                //    .SelectMany(m => m.Lessons)
    //                //    .Select(l => l.LessonContent)
    //                //    .OfType<VideoContent>()
    //                //    .Sum(v => (double?)v.DurationInSeconds) ?? 0 / 60.0),
    ...
    //            Curriculum = e.Course.Modules
    //                .OrderBy(m => m.Order)
    //                .Select(m => new ModuleSidebarDto
    //                {
    //                    ModuleId = m.ModuleId,
    //                    ModuleTitle = m.Title,
    ...
    //                    //TotalDurationInMinutes = (int)Math.Round(m.Lessons
    //                    //    .Select(l => l.LessonContent)
    //                    //    .OfType<VideoContent>()
    //                    //    .Sum(v => (double?)v.DurationInSeconds) ?? 0 / 60.0),
    ...
    //    // In Memeory calculations
    //    var lessonIds = classroomDto.Curriculum
    //        .SelectMany(m => m.Lessons)
    //        .Select(l => l.LessonId)
    //        .ToHashSet();

    //    var completedLessonsIds = await progressQuery
    //        .Where(p => lessonIds.Contains(p.LessonId) && p.IsCompleted)
    //        .Select(p => p.LessonId)
    //        .ToHashSetAsync();

    //    var activeLessonId = -1;
    //    var activeLessonIdx = 0;
    //    var activeModuleId = -1;
    //    var activeModuleIdx = 0;
    ...
    //        {
    //            if (completedLessonsIds.Contains(lesson.LessonId))
    //            {
    //                lesson.IsCompleted = true;
    //                classroomDto.CompletedLessonsCount++;
    //            }

    //            if (lessonId != null && lessonId == lesson.LessonId)
    //            {
    //                activeModuleId = module.ModuleId;
    //                activeModuleIdx = i;

    //                activeLessonId = (int)lessonId;
    //                activeLessonIdx = j;
    //            }
    //            else if (lessonId == null && lesson.IsCompleted)
    //            {
    ...
    //    classroomDto.Curriculum.ElementAt(activeModuleIdx).IsActive = true;
    //    classroomDto.Curriculum.ElementAt(activeModuleIdx).Lessons.ElementAt(activeLessonIdx).IsActive = true;
    ...
    //    foreach(var module in classroomDto.Curriculum)
    //        module.TotalDurationInMinutes = module.Lessons
    //            .Sum(l => l.TotalDurationInMinutes);

    //    classroomDto.CourseOverviewStats.TotalDurationInMinutes = classroomDto.Curriculum
    //        .Sum(m => m.TotalDurationInMinutes);

    //    classroomDto.ProgressPrercentage = (int)Math.Round(classroomDto.CompletedLessonsCount / (double)classroomDto.CourseOverviewStats.LessonsCount);
        

    //    var activeModule = classroomDto.Curriculum.ElementAt(activeModuleIdx);
    //    var activeLesson = activeModule.Lessons.ElementAt(activeLessonIdx);
    ...
    //            ContentUrl = l.LessonContent.ContentUrl,
    ...
    //                ResourceUrl = "TPH is broken fix it in the database"
    //            })
    //        })
    //        .FirstOrDefaultAsync();
