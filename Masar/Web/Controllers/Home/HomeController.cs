            ...
            var viewModel = new BrowseCoursesViewModel
            {
                Data = new BrowseResultViewModel<CourseBrowseCardViewModel>
                {
                    ...
                    Items = coursesData.Items.Select(c => new CourseBrowseCardViewModel
                    {
                        ...
                        MainCategory = c.MainCategory?.Name ?? "Uncategorized",  // FIX: Added null-conditional and null-coalescing operators
                        Categories = c.Categories?.Select(cat => cat.Name) ?? Enumerable.Empty<string>(),  // FIX: Added null checks
                        Languages = c.Languages?.Select(lang => lang.Name) ?? Enumerable.Empty<string>(),  // FIX: Added null checks
