...
(function () {
    ...
    function initPasswordToggle() {
        // نبحث عن كل الأزرار التي تملك الكلاس .toggle-password
        const toggleButtons = document.querySelectorAll('.toggle-password');

        toggleButtons.forEach(btn => {
            btn.addEventListener('click', () => {
                // الحقل المراد تغييره هو العنصر السابق للزر مباشرة (Input)
                const input = btn.previousElementSibling;
                const icon = btn.querySelector('i'); // أيقونة FontAwesome

                if (input.type === 'password') {
                    input.type = 'text'; // تحويل لنص مقروء
                    icon.classList.remove('fa-eye');
                    icon.classList.add('fa-eye-slash');
                } else {
                    input.type = 'password'; // إرجاع لنجوم
                    icon.classList.remove('fa-eye-slash');
                    icon.classList.add('fa-eye');
                }
            });
    ...
    // ========================================
    // 2. مقياس قوة كلمة المرور
    // ========================================

    function initPasswordStrength() {
        // تأكد أن الـ ID في الـ HTML هو registerPassword
        const passwordInput = document.getElementById('registerpassword');
        // تأكد أن عنصر عرض القوة يحمل الـ ID passwordStrength
        const strengthIndicator = document.getElementById('passwordStrength');
        ...
        passwordInput.addEventListener('input', () => {
            ...
            if (password.length > 0) {
                // إضافة الكلاس الجديد وتغيير النص
                strengthIndicator.classList.add(strength.class);
                // افترضنا وجود عنصر داخلي للنص، لو مش موجود غير السطر ده لـ strengthIndicator.textContent
                const textSpan = strengthIndicator.querySelector('.strength-text');
                if (textSpan) textSpan.textContent = strength.text;
                else strengthIndicator.textContent = strength.text;
            } else {
                // إعادة الوضع الافتراضي
                const textSpan = strengthIndicator.querySelector('.strength-text');
                if (textSpan) textSpan.textContent = 'Password strength';
                else strengthIndicator.textContent = '';
            }
        });
    }
    ...
    function calculatePasswordStrength(password) {
        ...
        if (password.length >= 8) score++;
        if (password.length >= 12) score++;
        if (/[a-z]/.test(password) && /[A-Z]/.test(password)) score++; // حروف كبيرة وصغيرة
        if (/\d/.test(password)) score++; // أرقام
        if (/[^a-zA-Z\d]/.test(password)) score++; // رموز خاصة
    ...
    function initPasswordMatch() {
        ...
        confirmInput.addEventListener('input', () => {
            if (confirmInput.value === '') {
                confirmInput.style.borderColor = ''; // العودة للون الأصلي (CSS)
            } else if (passwordInput.value === confirmInput.value) {
                confirmInput.style.borderColor = 'green'; // متطابق
            } else {
                confirmInput.style.borderColor = 'red'; // غير متطابق
            }
        });
    }
